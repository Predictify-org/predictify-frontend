...
function TimeRemainingProgress({ event }: { event: Event }) {
  ...
  const progressValue = Math.max(
    0,
    Math.min(100, (currentDays / maxDays) * 100),
  );
...
export function VirtualizedEventsList({
  ...
  const [keyboardFocusedRowId, setKeyboardFocusedRowId] = React.useState<
    string | null
  >(null);
  ...
  const handleRowFocusCapture = React.useCallback(
    (event: React.FocusEvent<HTMLDivElement>, rowId: string) => {
      if (hadKeyboardEventRef.current && event.target instanceof HTMLElement) {
  ...
  const handleRowBlurCapture = React.useCallback(
    (event: React.FocusEvent<HTMLDivElement>, rowId: string) => {
      const nextFocusedElement = event.relatedTarget as Node | null;

      if (
        !event.currentTarget.contains(nextFocusedElement) &&
        keyboardFocusedRowId === rowId
      ) {
        setKeyboardFocusedRowId(null);
      }
    },
  ...
  return (
    <div className={cn("relative space-y-4", className)}>
      ...
      >
        ...
        >
          {rowVirtualizer.getVirtualItems().map((virtualRow) => {
            ...
            return (
              ...
              >
                <div
                  className="virtualized-event-row flex items-center gap-4 rounded-md border-b border-[#540D8D] px-4 py-3 transition-[background-color,box-shadow] hover:bg-[#540D8D] md:px-6 md:py-4"
                  data-event-row-id={event.id}
                  data-focus-visible={
                    keyboardFocusedRowId === event.id ? "true" : "false"
                  }
                  onFocusCapture={(focusEvent) =>
                    handleRowFocusCapture(focusEvent, event.id)
                  }
                  onBlurCapture={(blurEvent) =>
                    handleRowBlurCapture(blurEvent, event.id)
                  }
                >
                  <button
                    type="button"
                    className="flex-1 cursor-pointer text-left focus-visible:outline-none"
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 text-white hover:bg-white/10 hover:text-white focus-visible:bg-white/10 focus-visible:text-white focus-visible:ring-0 focus-visible:ring-offset-0"
                      >
                        <MoreHorizontal className="h-4 w-4" />
                        <span className="sr-only">
                          Open actions menu for {event.title}
                        </span>
                      </Button>
                    </DropdownMenuTrigger>
        ...
        <EndOfList
          show={
            !hasNextPage && filteredEvents.length > 0 && !isFetchingNextPage
          }
