...
export function getWalletPrefs(): WalletPrefs {
  ...
  try {
    ...
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      console.warn("[walletPrefs] Stored wallet preferences have an invalid shape; using defaults.");
      return { ...DEFAULT_PREFS };
    }

    const prefs = parsed as Partial<WalletPrefs> & Record<string, unknown>;
    return {
      // Preserve unknown future fields so setWalletPrefs round-trips safely
      ...prefs,
      lastUsedWalletId: typeof prefs.lastUsedWalletId === "string" && prefs.lastUsedWalletId.length > 0 ? prefs.lastUsedWalletId : null,
      lastUsedWalletNetwork: typeof prefs.lastUsedWalletNetwork === "string" && prefs.lastUsedWalletNetwork.length > 0 ? prefs.lastUsedWalletNetwork : null,
    } as WalletPrefs;
  } catch (error) {
