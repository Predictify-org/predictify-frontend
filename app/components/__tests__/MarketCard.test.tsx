...
const SAMPLE_MARKET: Market = {
  ...
  activity24h: [
    10, 8, 4, 2, 1, 3, 8, 20, 40, 60, 75, 85,
    90, 88, 82, 78, 68, 55, 45, 35, 28, 20, 14, 10,
  ],
  status: "active",
}
...
describe("MarketCard", () => {
  ...
  it("updates the nudge when allowance changes in store", () => {
    act(() => {
      useUserLimitsStore.getState().setRemainingDailyAllowance("test-market", 45)
    })
    render(<MarketCard market={SAMPLE_MARKET} />)
    const nudge = screen.getByTestId("betting-limit-nudge")
  ...
  it("applies animation delay based on index", () => {
    render(<MarketCard market={SAMPLE_MARKET} index={2} reducedMotion={false} />)
