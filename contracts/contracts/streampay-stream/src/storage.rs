...
fn extend_persistent_ttl(env: &Env, key: &DataKey) {
    env.storage().persistent().extend_ttl(key, STREAM_TTL_MIN_REMAINING, STREAM_TTL_EXTEND_TO);
}

fn extend_instance_ttl(env: &Env) {
    env.storage().instance().extend_ttl(INSTANCE_TTL_MIN_REMAINING, INSTANCE_TTL_EXTEND_TO);
}
